...
from model import MMRest
from PyMMLCNLoss import PyMMLCNLoss
from optimizers import PSDAdam


class ConsoleFileTee(io.TextIOBase):
    def __init__(self, stream: io.TextIOBase, log_file):
        super().__init__()
        self.stream = stream
...
class Solver(object):
    def __init__(self, hyp_params, train_loader, dev_loader, test_loader, is_train=True, model=None, pretrained_emb=None):
        ...
        self.p_value =[]
        self.t_statistic = []
        self.c_sim_loss = PyMMLCNLoss(v1=hp.v1, v2=hp.v2, xi=hp.xi, alpha_=hp.alpha_, max_clusters=hp.k)
        self.update_batch = hp.update_batch

        # initialize the model
        if model is None:
            self.model = model = MMRest(hp)
        ...
        self.c_sim_loss.to(self.device)

        self.log_dir = Path(__file__).resolve().parent / 'logs'
        self.log_dir.mkdir(parents=True, exist_ok=True)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        self.log_path = self.log_dir / f"{self.hp.dataset}_{timestamp}.log"
        ...
        if self.is_train:
            self._prepare_metric_optimizer()

        self.scheduler_main = ReduceLROnPlateau(
            self.optimizer_main, mode='min', patience=hp.when, factor=0.5, verbose=True)

    def _prepare_metric_optimizer(self):
        if self.optimizer_metric is not None:
            return

        feat_layer = getattr(self.model, 'clus_text', None)
        feat_dim = getattr(feat_layer, 'out_features', None) if feat_layer is not None else None
        if feat_dim is None:
            raise ValueError('Unable to determine feature dimension for PyMMLCNLoss parameters.')

        self.c_sim_loss._ensure_params(feat_dim, self.device)

        metric_params = [p for p in self.c_sim_loss.parameters() if p.requires_grad]
        if not metric_params:
            return

        self.optimizer_metric = PSDAdam(
            metric_params,
            lr=self.hp.lr_main,
            betas=(0.9, 0.999),
            eps=1e-8,
            weight_decay=self.hp.weight_decay_main
        )


    def _ensure_log_redirect(self):
        if self._log_file is not None:
            return
        self._log_file = open(self.log_path, 'a', encoding='utf-8')
        self._stdout_logger = ConsoleFileTee(self._original_stdout, self._log_file)
        self._stderr_logger = ConsoleFileTee(self._original_stderr, self._log_file)
        ...
        try:
            ...
            def train(model, optimizer, criterion, epochs, stage=1):
                ...
                for i_batch, batch_data in enumerate(self.train_loader):
                    ...
                    optimizer.zero_grad()
                    if optimizer_metric is not None:
                    ...
                    preds, clus_text, clus_visual, clus_acoustic = model(
                        text, visual, audio, vlens, alens,
                        bert_sent, bert_sent_type, bert_sent_mask, y, self.c_sim_loss)
                    feats_c = torch.cat([clus_text, clus_visual, clus_acoustic], dim=0)  # [3B, D]
                    labels_rep_c = y.repeat(3, 1).view(-1)  # [3B]
                    loss_mmrest = self.c_sim_loss(feats_c, labels_rep_c)
                    if stage == 1:
                        y_loss = criterion(preds, y)

                        if self.hp.contrast:
                            loss = y_loss + 0.01 * loss_mmrest
                        else:
                            loss = y_loss + 0.01 * loss_mmrest
                        loss.backward()
                    ...
                    if left_batch == 0:
                        ...
                        if optimizer_metric is not None:
                            torch.nn.utils.clip_grad_norm_(
                                list(self.c_sim_loss.parameters()), self.hp.clip)
                    ...
                    if i_batch % self.hp.log_interval == 0 and i_batch > 0:
                        ...
                        variance_msg = self._format_cluster_variance_log(prefix=" | ", empty_message='')
                        base_line = 'Epoch {:2d} | Batch {:3d}/{:3d} | Time/Batch(ms) {:5.2f} | Train Loss ({}) {:5.4f} | BA {:.4f}'. \
                            format(epochs, i_batch, num_batches, elapsed_time * 1000 / self.hp.log_interval,
                                'TASK+BA+CPC' if stage == 1 else 'Neg-lld', avg_loss, avg_ba)
                        print(base_line + variance_msg)
                        proc_loss, proc_size = 0, 0
                        ba_loss = 0.0
                        start_time = time.time()
            ...
            def evaluate(model, criterion, epochs, test=False):
                ...
                with torch.no_grad():
                    for batch in loader:
                        ...
                        preds, _, _, _ = model(
                            text, vision, audio, vlens, alens, bert_sent, bert_sent_type, bert_sent_mask, self.c_sim_loss)
            ...
            for epoch in range(1, self.hp.num_epochs + 1):
                ...
                variance_line = self._format_cluster_variance_log(prefix="    ", empty_message='    K=0 (no clusters cached)')
                if variance_line:
                    print(variance_line)
            ...
            torch.save(self.p_value, "mosi_mmrest_p_value.pth")
            torch.save(self.t_statistic, "mosi_mmrest_t_statistic.pth")
