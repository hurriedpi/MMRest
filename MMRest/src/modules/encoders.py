...
MODEL_PATH = '/mnt/g/Pretrained Model/bert-base-uncased' 
...
def get_resNet():
    b1 = nn.Sequential(nn.Conv1d(768, 50, kernel_size=1, stride=1),
                       nn.ReLU())
...
def get_resNet2():
    b1 = nn.Sequential(nn.Conv1d(64, 50, kernel_size=1, stride=1),
                       nn.ReLU())
    # b2 = nn.Sequential(*resnet_block(512, 256, 1),
    #                    nn.ReLU())
    # b3 = nn.Sequential(*resnet_block(256, 128, 1),
...
def get_resNet4():
    b1 = nn.Sequential(nn.Conv1d(896, 50, kernel_size=1, stride=1),
                       nn.ReLU())
    # b2 = nn.Sequential(*resnet_block(512, 256, 1),
    #                    nn.ReLU())
    # b3 = nn.Sequential(*resnet_block(256, 128, 1),
